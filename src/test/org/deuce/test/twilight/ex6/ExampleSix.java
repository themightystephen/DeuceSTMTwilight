...
public class ExampleSix {
	public int counter1 = 0; // pretend counter is a shared variable that is accessed by many threads/transactions (actually, it is properly shared if we have multiple threads invoking example() on the same object)
	...
	@Atomic //(retries = 100, twilight = false)
	public void incOneThenTwo() {
		System.out.println(Thread.currentThread()+" Transaction (re)started.");
		counter1++;
		counter2++;
	}

	@Atomic
	public void incTwoThenOne() {
		System.out.println(Thread.currentThread()+" Transaction (re)started.");
		counter2++;
		counter1++;
	}
